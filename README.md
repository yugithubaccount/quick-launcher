...
| **面板管理** | 左侧列表最后一行的「**＋ 新建面板…**」直接建空面板；在左侧面板上**右键**可 新建 / 重命名 / 删除空面板 / 把选中的工具移动到这一栏；条目右键或工具菜单里的「**移动到面板…**」可把工具换到别的面板（下拉选已有或输入新名字） |
...
| 视图切换 | `Ctrl+1` 列表视图、`Ctrl+2` 大图标网格；配置里 `#VIEW=icon` 可设默认视图；窗口拉大/最大化后大图标会按新宽度**自动重排** |
...
| 想做的事 | 怎么做 |
|---|---|
| 新建一个面板 | 左侧列表最后一行的「**＋ 新建面板…**」→ 输入名字 → 确定（先建成空的，之后再往里加工具）；也可以在「＋ 添加工具」时在“面板”那一栏直接输入新名字 |
| 往已有面板加工具 | 先在左侧选中那个面板 → 把文件**拖进窗口**，或用「＋ 添加工具」 |
| 把工具换到别的面板 | 选中条目 → 右键「**移动到面板…**」（或工具菜单 `Alt+T` → `M`）；也可以先选中条目，再**右键左侧的目标面板** →「把选中的工具移动到此面板」 |
| 重命名面板 | 在左侧那个面板上**右键** →「重命名此面板…」（里面的条目会一起改名；新名字是已有面板时两个面板合并） |
| 删除面板 | 面板里的条目删光后它就自己消失了；空面板可以右键 →「删除此面板（仅空面板）」 |
| 调整面板顺序 | 面板按配置里**首次出现的顺序**排；想固定顺序就手工调 `tools_utf8.txt` 里各行的先后（内置「系统面板」永远排第一） |

> 界面上新建的空面板会在配置里记成一行 `#PANEL=面板名` —— 这就是它“没有任何条目也能留在左侧”的原因；
> 不要了就在界面上右键删掉空面板（或手工删掉那一行）。
> 内置的「系统面板」不能改名 / 删除，要隐藏写 `#SYSPANEL=0`。

![新建面板](docs/screenshots/新建面板对话框.png)
...
| **「＋ 新建面板…」（v1.5）** | ✅ 无头 X 下真点击左栏最后一行 → 输入中文名 → 确定：左栏立刻出现 `网络工具 (0)`、配置写入 `#PANEL=网络工具`（重启后仍在，实测连建两个都保留）；重名不会重复写行 |
| **移动到面板（v1.5）** | ✅ 选中「演示 PowerShell」→ 工具菜单 `Alt+T` `M` → 下拉选 `网络工具` → 确定：配置该行面板名改为 `网络工具`，列表切到该面板并保持选中（网络工具 1 / 本目录示例 1） |
| **面板右键菜单（v1.5）** | ✅ 在左栏某个面板上右键：新建面板 / 把选中的工具移动到此面板 / 重命名此面板 / 删除此面板；内置「系统面板」上后三项灰掉 |
| **大图标重排（v1.5）** | ✅ 窗口 1012 px 宽时每行 8 个图标；`xdotool windowsize` 拉到 1264 px 宽后自动重排成每行 9 个、总行数减少（修的就是"最大化后图标不重排"） |
...
- **大图标视图（`LVS_ICON`）窗口变大后不会自动重排**：图标留在原位、右边空一片。要在 `WM_SIZE`/`layout()` 之后补一次 `SendMessageW(hList, LVM_ARRANGE, LVA_DEFAULT, 0)`（并把 `LVS_AUTOARRANGE` 加进图标视图的样式），切换面板/刷新列表后也要再排一次。
- 左侧面板列表把「＋ 新建面板…」做成了**同一 ListBox 里的动作行**：`LBN_SELCHANGE` 里要单独判断这一行的下标、执行动作、再用 `LB_SETCURSEL` 把选中项**还原**，否则界面上会停在一个不存在的面板上。
- **空面板必须另外记住**：面板是由条目聚合出来的，没有任何条目就不会出现；界面上新建的空面板写成配置里的 `#PANEL=名称` 行，保存时**要重新写出来**（和 `#SYSPANEL`/`#NOCACHE` 一样，忘了回写就丢）。
